...
class HorizontalCylinderDetector(Node):
    def __init__(self):
        ...
        # --- NEW: Flag to ensure warning is only logged once ---
    ...
    def image_callback(self, msg):
        ...
        if self.latest_depth_image is None or not self.intrinsics_received:
            # --- FIXED: Manually implement "warn_once" logic ---
            if not self.warned_about_intrinsics:
                self.get_logger().warn('Camera intrinsics or depth image not yet received. Cannot compute 3D coordinates.')
                self.warned_about_intrinsics = True # Set flag so we don't warn again
            return
